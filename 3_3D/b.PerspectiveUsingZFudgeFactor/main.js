...
try {
  ...
  //Set up the uniform 'u_zFudgeFactor'
  const zFudgeFactorUniform = new UniformClass(gl, program, 'u_zFudgeFactor_f', 'uniform1f');
  ...
  // Define the initial values for transform parameters
  let translation = [0, 0, 0];
  let rotation  = [0, 0, 0];
  let scale = [1, 1, 1];
  let zFudgeFactor = 0;

  // Define Ortho projection parameters
  const near = 400;
  const far = -400;
  // Compute the projection matrix
  const projection_m4 = TransformsClass.getOrthoMatrix(
    0,
  ...
  function drawScene(){
    ...
    m4_multiply(transform_m4, projection_m4, translate_m4);
    ...
    // Set the new zFudgeFactor by assigning the uniform 'u_zFudgeFactor'
    zFudgeFactorUniform.setData(zFudgeFactor);
  ...
  const z_fudgefactor_slider = document.getElementById('z_fudgefactor');
  const z_fudgefactor_output = document.getElementById('z_fudgefactor_output');
  z_fudgefactor_slider.step = 0.2;
  z_fudgefactor_slider.min = 0;
  z_fudgefactor_slider.max = 2;

  z_fudgefactor_slider.value = zFudgeFactor;
  z_fudgefactor_output.textContent = zFudgeFactor;

  z_fudgefactor_slider.addEventListener('input', ()=> {
    zFudgeFactor = +z_fudgefactor_slider.value;
    z_fudgefactor_output.textContent = z_fudgefactor_slider.value;
    drawScene();
  });
  ...
  x_translate_slider.min = 0;
  x_translate_slider.max = gl.canvas.width;
  x_translate_slider.step = 2;
  x_translate_slider.value = translation[0];
  x_translate_output.textContent = translation[0];

  y_translate_slider.min = 0;
  y_translate_slider.max = gl.canvas.height;
  y_translate_slider.step = 2;
  y_translate_slider.value = translation[1];
  y_translate_output.textContent = translation[1];

  z_translate_slider.min = -gl.canvas.height;
  z_translate_slider.max = gl.canvas.height;
  z_translate_slider.step = 2;
